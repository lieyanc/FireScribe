...
export function ProjectDetailPage() {
  ...
  return <div className="flex flex-col gap-6">
    <PageHeader title={item.name} description={item.description || "尚未填写项目说明。"}>
      <Button variant="outline" onClick={() => setEditOpen(true)}><Pencil data-icon="inline-start" />编辑</Button>
      <Button variant="outline" onClick={() => setDeleteOpen(true)}><Trash2 data-icon="inline-start" />删除</Button>
    </PageHeader>
    ...
    <Card><CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle>项目文档</CardTitle><CardDescription>导出时将按照下列顺序合并。</CardDescription></div><Button onClick={() => setAddOpen(true)}><FilePlus2 data-icon="inline-start" />添加文档</Button></CardHeader><CardContent className="p-0">
