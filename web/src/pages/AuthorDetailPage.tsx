...
export function AuthorDetailPage() {
  ...
  return <div className="flex flex-col gap-6">
    <PageHeader title={item.name} description="作者笔迹档案会作为可审计的识别上下文快照写入每次 OCR run。">
      <Button asChild variant="outline"><a href={`/api/author-profiles/${profileID}/training-data`}><Download />下载 JSONL</a></Button>
      <Button variant="outline" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? <Spinner /> : <RefreshCw />}同步样本</Button>
    </PageHeader>
    ...
    <AlertDialog open={deleteOpen} onOpenChange={(open) => !destroy.isPending && setDeleteOpen(open)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>删除作者档案“{item.name}”？</AlertDialogTitle><AlertDialogDescription>关联文档不会被删除。</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={destroy.isPending} onClick={(e) => { e.preventDefault(); destroy.mutate(); }}>{destroy.isPending && <Spinner />}删除档案</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
