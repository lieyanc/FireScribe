...
export function JobsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {cancel.error ? <p className="text-sm text-destructive">{cancel.error.message}</p> : null}
      {retry.error ? <p className="text-sm text-destructive">{retry.error.message}</p> : null}
      <section className="panel overflow-hidden">
        <div className="jobs-grid border-b bg-muted/50 px-3 py-2 text-xs font-medium text-muted-foreground">
          <div>类型</div>
          <div>状态</div>
          <div>次数</div>
          <div>创建</div>
        ...
        {jobs.data?.length ? (
          jobs.data.map((job) => (
            <div key={job.id} className="jobs-grid items-center border-b px-3 py-3 text-sm transition-colors last:border-b-0 hover:bg-muted/40">
              <div className="min-w-0">
                <div className="truncate font-medium">{job.type}</div>
                {job.last_error ? <div className="mt-1 truncate text-xs text-destructive">{job.last_error}</div> : null}
              </div>
