...
// Profile/Adapter secrets in a mode-0600 local file. Database rows retain only
...
func (s *Store) ConfigureSecretFile(ctx context.Context, path string) error {
	...
	if s.secretPath == path && s.secrets.RecognizerProfiles != nil && s.secrets.ProviderAdapters != nil {
		return nil
	}
	secrets := storedSecrets{RecognizerProfiles: map[string]string{}, ProviderAdapters: map[string]string{}}
