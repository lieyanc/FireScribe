...
所有功能均需登录使用。首次启动（用户表为空）时，网页会引导创建第一个管理员账户；之后通过登录页进入。

- **管理员**：可使用全部功能，并管理设置、Prompt 版本库、识别档案、供应商适配器、OTA 更新以及用户账户（创建、编辑、禁用、重置密码、删除）。
- **普通用户**：仅可使用工作台功能（导入、识别、校对、搜索、导出等），侧边栏不显示管理入口。

账户与会话保存在 SQLite（`users` / `auth_sessions` 表）中；密码使用 PBKDF2-SHA256 哈希，会话通过 HttpOnly Cookie（30 天滑动过期）维持。系统始终保留至少一名启用的管理员：最后一名管理员无法被降级、禁用或删除；用户也不能禁用或删除自己的账户。修改密码会使其他设备的登录状态失效。

无头自动化仍可使用 `config.json` 中的 `update.admin_token`：携带 `X-Admin-Token`（或 Bearer）访问管理与更新接口；未配置令牌时，更新接口保留仅限本机回环地址的免登录访问，供 OTA 脚本本地验证使用。
